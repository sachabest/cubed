...
public class GameCenterTurnBasedBinding
{
	...
	// Saves the current match data without ending the turn or match
	...
	// iOS 6+ only! Ends the current participant's turn. You may update the matchOutcome for any GKTurnBasedParticipants that you wish to before ending the turn.
	// null or an empty array is a valid for playerIds.  If null is used then the next unassigned player will be set as the next participant
	...
	public static void findMatchProgramaticallyWithFilters( int minPlayers, int maxPlayers, string[] playersToInvite, uint playerGroup, uint playerAttributes )
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
		{
			string playerIdString = null;
	...
	[DllImport("__Internal")]
	private static extern void _gcTurnBasedChangeCurrentMatch( string matchId );

	// Sets the currently active match
	public static void changeCurrentMatch( string matchId )
	{
		if( Application.platform == RuntimePlatform.IPhonePlayer )
			_gcTurnBasedChangeCurrentMatch( matchId );
	}
